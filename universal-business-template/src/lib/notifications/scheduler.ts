...
export function shouldRunNow(frequency: NotificationFrequency, lastRun: Date | null, now = new Date()): boolean {
...
export function formatFrequency(frequency: NotificationFrequency): string {
  ...
  if (frequency.type === 'weekly') return `Săptămânal, ${DAYS[frequency.dayOfWeek]} la ${frequency.hour}:00`;
  if (frequency.type === 'monthly') return `Lunar, ziua ${frequency.dayOfMonth} la ${frequency.hour}:00`;
  return 'Necunoscut';
}
